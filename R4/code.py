...
REWARDS = [(0.5,-0.5,0),(1,-1,0),(2,-1,0),(2,-2,0),(10,-1,0),(10,-10,0),(100,-100,0)]

MAX_GAMES = 60000
update_every = 1000

total_win = np.zeros((len(REWARDS),int(MAX_GAMES/update_every)))
total_lose = np.zeros((len(REWARDS),int(MAX_GAMES/update_every)))
total_draw = np.zeros((len(REWARDS),int(MAX_GAMES/update_every)))
epsilons = np.zeros(len(REWARDS))
reward_iteration_counter = -1
for R in REWARDS:
    reward_iteration_counter+=1
    Q1 = {}
    Q2 = {}   
    ...
    player2_state = 0
    player2_move = 0
    win_reward , lose_reward , draw_reward = R
    last_Q1_len = 1
    last_Q2_len = 1
    eps_zero_game = 0
    counter = 0
    while game_number < MAX_GAMES:
        if (game_number+1)%update_every == 0:
            if len(Q1.keys())/last_Q1_len == 1:
                epsilon = 0
                if counter == 0:
                    eps_zero_game = game_number
                    epsilons[reward_iteration_counter] = eps_zero_game
                counter+=1
    ...
    plt.plot(np.array(win_rate1)*100/update_every,label =rf'P1-win-$r={win_reward}-{lose_reward},{draw_reward}$')
    plt.plot(np.array(win_rate2)*100/update_every,label='P2-win')
    plt.plot(np.array(draw_rate)*100/update_every , label='Draw')
    plt.plot(np.ones(50)*eps_zero_game/100,np.linspace(-2,102,50),'r--',label=r'$\epsilon = 0$')
    total_win[reward_iteration_counter] = np.array(win_rate1)*100/update_every
    total_lose[reward_iteration_counter] = np.array(win_rate2)*100/update_every
    total_draw[reward_iteration_counter] = np.array(draw_rate)*100/update_every
...
for i in range(len(REWARDS)):
    plt.plot(total_win[i],label =f'r={REWARDS[i]}')

plt.legend(fontsize=20)
plt.xlabel(r'Played games$\times 10^{3}$', fontsize=30)
plt.ylabel('Win P1%',fontsize=30)
plt.xticks(fontsize=30)
plt.yticks(fontsize=30)
plt.ylim(-2,102)
#plt.xlim(0,60)
...
plt.figure(figsize=(15,10))
for i in range(len(REWARDS)):
    plt.plot(total_lose[i],label =f'r={REWARDS[i]}')

plt.legend(fontsize=20)
plt.xlabel(r'Played games$\times 10^{3}$', fontsize=30)
plt.ylabel('Win P2%',fontsize=30)
plt.xticks(fontsize=30)
plt.yticks(fontsize=30)
plt.ylim(-2,102)
#plt.xlim(0,60)
...
plt.figure(figsize=(15,10))
for i in range(len(REWARDS)):
    plt.plot(total_draw[i],label =f'r={REWARDS[i]}')

plt.legend(fontsize=20)
plt.xlabel(r'Played games$\times 10^{3}$', fontsize=30)
plt.ylabel('Draw %',fontsize=30)
plt.xticks(fontsize=30)
plt.yticks(fontsize=30)
plt.ylim(-2,102)
#plt.xlim(0,60)
